...
def collect(domain_path: str, source: str, store: bool = True, requester: Optional[str] = None) -> Optional[dict]:
    ...
    try:
        ...
        if doc.error:
            ...
            return _create_failed_report(domain_path, source, requested_at, requester, run_id, events, source_url=doc.source_url, connector_used=connector.__class__.__name__)
        ...
        if store and parsed_data:
            try:
                ...
                record = KnowledgeRecord(
                    source=f"{domain_path}_{source}", collected_at=datetime.now(), raw_data=doc.content, parsed_data=parsed_data,
        ...
        report = {
            ...
            "metadata": {"collected_at": datetime.now().isoformat() + "Z", "source_url": doc.source_url, "content_type": doc.content_type, "size_bytes": len(doc.content) if doc.content else 0},
            "provenance": {"run_id": run_id, "collector_version": "1.0.0", "domain_version": "1.0.0"}
        }
        if record_id:
...
def _create_failed_report(domain_path, source, requested_at, requester, run_id, events, error=None, source_url=None, connector_used=None):
    """Create a failed report while preserving available execution evidence."""
    return {
        "report_version": "1.0.0",
        "request": {"domain_path": domain_path, "source": source, "requested_at": requested_at, "requester": requester},
        "data": {"raw": None, "parsed": None, "format": None},
        "execution": {"status": "failed", "duration_ms": 0, "events": events, "connector_used": connector_used},
        "metadata": {"collected_at": datetime.now().isoformat() + "Z", "source_url": source_url, "content_type": None, "size_bytes": 0},
        "provenance": {"run_id": run_id, "collector_version": "1.0.0", "domain_version": "1.0.0"}
    }
...
def main():
    ...
    collect_parser.add_argument("domain", help="Domain path")
    collect_parser.add_argument("source", help="Source identifier")
