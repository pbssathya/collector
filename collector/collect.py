...
def collect(domain_path: str, source: str, store: bool = True, requester: Optional[str] = None) -> Optional[dict]:
    ...
    if not connector:
        return {
            ...
            "execution": {
                ...
                "events": [{"type": "domain_not_found", "message": f"Unknown domain: {domain_path}", "timestamp": datetime.now().isoformat() + "Z"}],
                "connector_used": None
            },
    ...
    try:
        ...
        if doc.error:
            ...
            return _create_failed_report(domain_path, source, requested_at, requester, run_id, events, error=doc.error, source_url=doc.source_url, connector_used=connector.__class__.__name__)
